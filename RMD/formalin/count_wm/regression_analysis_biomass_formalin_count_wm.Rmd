...
```{r}
biomass_formalin_count_wm_cleaned <- read_csv(here("data/processed/formalin/count_wm/biomass_formalin_count_wm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_formalin_count_wm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_object = map(.x = data,
                          ~ lm(wm_g ~ individual_count, data = .x)))
```
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_formalin_count_wm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_formalin_count_wm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                            .f = ~ lm(log(wm_g) ~ log(individual_count), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(individual_count), y = log(wm_g))) + 
                           geom_point() +
                           geom_smooth(method = "lm") +
                           labs(x = "log(e) individual count", y = "log(e) wet mass (g)") +
                           ggtitle(.y)))
...
write_csv(reg_summary_a_sf,
          here("output/formalin/count_wm/biomass_regression_summary_formalin_count_wm.csv"))
```
...
taxa_nested_lm_ln_plot_coef_CI_BT <- taxa_nested_lm_ln_plot_coef_CI |>
  ...
  select(taxa_clean, individual_count, wm_g, lower_CI_line, upper_CI_line, sf) |>
  nest(data = c(individual_count, wm_g, lower_CI_line, upper_CI_line)) |>
  right_join(reg_summary_a) |>
  mutate(BT_plot = pmap(.l = list(data, slope, intercept, sf, taxa_clean, r2),
                        .f = ~ggplot(..1, aes(x = individual_count, y = wm_g)) +
                          ...
                          geom_ribbon(aes(ymin = lower_CI_line, ymax = upper_CI_line), fill = "darkgreen", 
                                      alpha = 0.2) +
                          ggtitle(..5) + 
                          labs(x = "individual count", y = "wet mass (g)") + 
                          geom_text(x = 0.25*max(..1$individual_count), y = 0.85*max(..1$wm_g), 
                                    label = deparse(bquote(paste("W = ", 
                                                                 ...
                                                                 sep = ""))),
                                    parse = TRUE, size = 4) +
                          geom_text(x = 0.25*max(..1$individual_count), y = 0.75*max(..1$wm_g), 
...
```{r}
biomass_formalin_count_wm_cleaned_ratios <- biomass_formalin_count_wm_cleaned |>
...
write_csv(biomass_formalin_count_wm_cleaned_ratios,
          here("output/formalin/count_wm/biomass_ratio_summary_formalin_count_wm.csv"))
```
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_formalin_count_wm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                          ~ lm(log(wm_g) ~ log(individual_count), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(individual_count), y = log(wm_g))) + 
                           geom_point() +
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/formalin/count_wm/biomass_regression_summary_formalin_count_wm_outliers_removed.csv"))
```
...
```{r}
biomass_formalin_count_wm_outliers_removed_ratios <- biomass_formalin_count_wm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(wm_count = wm_g/individual_count) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_formalin_count_wm_outliers_removed_ratios,
          here("output/formalin/count_wm/biomass_ratio_summary_formalin_count_wm_outliers_removed.csv"))
```

### combining summary tables

```{r}
summary_formalin_count_wm_ratios <- left_join(reg_summary_a_sf, biomass_formalin_count_wm_cleaned_ratios)

summary_formalin_count_wm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_formalin_count_wm_outliers_removed_ratios) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_formalin_count_wm <- bind_rows(summary_formalin_count_wm_ratios, summary_formalin_count_wm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_formalin_count_wm,
          here("output/formalin/count_wm/full_summary_formalin_count_wm.csv"))
```
...
```{r}
pred_mass_formalin_count_wm_outliers_removed_MAPE <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_wm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_wm = map2(.x = data,
                              .y = biomass_formalin_count_wm_outliers_removed_ratios$mean_wm_count,
...
pred_mass_formalin_count_wm_outliers_removed_MAPE |> 
