...
```{r}
biomass_formalin_count_wm_ratios <- biomass_formalin_count_wm_cleaned |>
  mutate(wm_count = wm_g/individual_count) |>
  group_nest(taxa_clean) |>
  mutate(wm_count_t_test = map(.x = data,
                            .f = ~ t.test(.x$wm_count)),
         ci_mean_wm_count = map(.x = wm_count_t_test,
                             .f = ~ .x$conf.int)) |>
  unnest(ci_mean_wm_count) |>
  mutate(ci_bound = c(rep(c("lower", "upper"), times = length(taxa_clean)/2))) |>
  pivot_wider(names_from = ci_bound, values_from = ci_mean_wm_count) |>
  unnest(data) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
            mean_wm_count = mean(wm_count),
            lower_ci_mean_wm_count = mean(lower),
            upper_ci_mean_wm_count = mean(upper),
...
write_csv(biomass_formalin_count_wm_ratios,
...
```{r}
pred_mass_formalin_count_wm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_wm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_wm = map2(.x = data,
                              .y = biomass_formalin_count_wm_ratios$mean_wm_count,
...
```{r}
pred_mass_formalin_count_wm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_wm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$individual_count)^..3*..4),
         ols_pred_wm_percent_error = map2(.x = data,
                                          .y = ols_pred_wm,
...
```{r}
summary_formalin_count_wm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_formalin_count_wm_ratios) |>
  left_join(pred_mass_formalin_count_wm_error_eval)
...
summary_formalin_count_wm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
  ...
  left_join(pred_mass_formalin_count_wm_outliers_removed_error_eval) |>
