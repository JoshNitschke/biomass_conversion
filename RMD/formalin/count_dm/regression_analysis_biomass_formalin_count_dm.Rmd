...
```{r}
biomass_formalin_count_dm_ratios <- biomass_formalin_count_dm_cleaned |>
  mutate(dm_count = dm_g/individual_count) |>
  group_nest(taxa_clean) |>
  mutate(dm_count_t_test = map(.x = data,
                            .f = ~ t.test(.x$dm_count)),
         ci_mean_dm_count = map(.x = dm_count_t_test,
                             .f = ~ .x$conf.int)) |>
  unnest(ci_mean_dm_count) |>
  mutate(ci_bound = c(rep(c("lower", "upper"), times = length(taxa_clean)/2))) |>
  pivot_wider(names_from = ci_bound, values_from = ci_mean_dm_count) |>
  unnest(data) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
            mean_dm_count = mean(dm_count),
            lower_ci_mean_dm_count = mean(lower),
            upper_ci_mean_dm_count = mean(upper),
...
write_csv(biomass_formalin_count_dm_ratios,
...
```{r}
pred_mass_formalin_count_dm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_formalin_count_dm_ratios$mean_dm_count,
...
```{r}
pred_mass_formalin_count_dm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$individual_count)^..3*..4),
         ols_pred_dm_percent_error = map2(.x = data,
                                          .y = ols_pred_dm,
...
```{r}
summary_formalin_count_dm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_formalin_count_dm_ratios) |>
  left_join(pred_mass_formalin_count_dm_error_eval)
...
summary_formalin_count_dm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
  ...
  left_join(pred_mass_formalin_count_dm_outliers_removed_error_eval) |>
