...
```{r}
biomass_formalin_count_dm_cleaned <- read_csv(here("data/processed/formalin/count_dm/biomass_formalin_count_dm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_formalin_count_dm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_object = map(.x = data,
                          ~ lm(dm_g ~ individual_count, data = .x)))
```
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_formalin_count_dm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_formalin_count_dm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                            .f = ~ lm(log(dm_g) ~ log(individual_count), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(individual_count), y = log(dm_g))) + 
                           geom_point() +
                           geom_smooth(method = "lm") +
                           labs(x = "log(e) individual count", y = "log(e) dry mass (g)") +
                           ggtitle(.y)))
...
write_csv(reg_summary_a_sf,
          here("output/formalin/count_dm/biomass_regression_summary_formalin_count_dm.csv"))
```
...
taxa_nested_lm_ln_plot_coef_CI_BT <- taxa_nested_lm_ln_plot_coef_CI |>
  ...
  select(taxa_clean, individual_count, dm_g, lower_CI_line, upper_CI_line, sf) |>
  nest(data = c(individual_count, dm_g, lower_CI_line, upper_CI_line)) |>
                        ...
                        .f = ~ggplot(..1, aes(x = individual_count, y = dm_g)) +
                          ...
                          geom_text(x = 0.25*max(..1$individual_count), y = 0.85*max(..1$dm_g), 
                                    label = deparse(bquote(paste("W = ", 
                                                                 ...
                                                                 sep = ""))),
                                    parse = TRUE, size = 4) +
                          geom_text(x = 0.25*max(..1$individual_count), y = 0.75*max(..1$dm_g), 
...
```{r}
biomass_formalin_count_dm_cleaned_ratios <- biomass_formalin_count_dm_cleaned |>
...
write_csv(biomass_formalin_count_dm_cleaned_ratios,
          here("output/formalin/count_dm/biomass_ratio_summary_formalin_count_dm.csv"))
```
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_formalin_count_dm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                          ~ lm(log(dm_g) ~ log(individual_count), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(individual_count), y = log(dm_g))) + 
                           geom_point() +
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/formalin/count_dm/biomass_regression_summary_formalin_count_dm_outliers_removed.csv"))
```
...
```{r}
biomass_formalin_count_dm_outliers_removed_ratios <- biomass_formalin_count_dm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(dm_count = dm_g/individual_count) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_formalin_count_dm_outliers_removed_ratios,
          here("output/formalin/count_dm/biomass_ratio_summary_formalin_count_dm_outliers_removed.csv"))
```

### combining summary tables

```{r}
summary_formalin_count_dm_ratios <- left_join(reg_summary_a_sf, biomass_formalin_count_dm_cleaned_ratios)

summary_formalin_count_dm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_formalin_count_dm_outliers_removed_ratios) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_formalin_count_dm <- bind_rows(summary_formalin_count_dm_ratios, summary_formalin_count_dm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_formalin_count_dm,
          here("output/formalin/count_dm/full_summary_formalin_count_dm.csv"))
```
...
```{r}
pred_mass_formalin_count_dm_outliers_removed_MAPE <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_formalin_count_dm_outliers_removed_ratios$mean_dm_count,
...
pred_mass_formalin_count_dm_outliers_removed_MAPE |> 
