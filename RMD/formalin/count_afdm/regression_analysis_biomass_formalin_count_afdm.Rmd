...
```{r}
biomass_formalin_count_afdm_cleaned <- read_csv(here("data/processed/formalin/count_afdm/biomass_formalin_count_afdm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_formalin_count_afdm_cleaned |> 
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_formalin_count_afdm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_formalin_count_afdm_cleaned |> 
  group_nest(taxa_clean) |>
...
reg_summary <- add_column(long_summary_short, tibble(p, slope, slope_se, intercept, intercept_se))
...
write_csv(reg_summary_a_sf,
          here("output/formalin/count_afdm/biomass_regression_summary_formalin_count_afdm.csv"))
```
...
```{r}
biomass_formalin_count_afdm_cleaned_ratios <- biomass_formalin_count_afdm_cleaned |>
  ...
  summarise(n = n(),
            ...
            range_count = paste0(min(individual_count), 
                              "-", max(individual_count)))

# Saving the ratio summary
write_csv(biomass_formalin_count_afdm_cleaned_ratios,
          here("output/formalin/count_afdm/biomass_ratio_summary_formalin_count_afdm.csv"))
```
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_formalin_count_afdm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
...
reg_summary_outliers_removed <- add_column(long_summary_outliers_removed_short, 
                                           tibble(p, slope, slope_se, intercept, intercept_se))
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/formalin/count_afdm/biomass_regression_summary_formalin_count_afdm_outliers_removed.csv"))
```
...
```{r}
biomass_formalin_count_afdm_outliers_removed_ratios <- biomass_formalin_count_afdm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(afdm_count = afdm_g/individual_count) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_formalin_count_afdm_outliers_removed_ratios,
          here("output/formalin/count_afdm/biomass_ratio_summary_formalin_count_afdm_outliers_removed.csv"))
```

### combining summary tables

```{r}
summary_formalin_count_afdm_ratios <- left_join(reg_summary_a_sf, biomass_formalin_count_afdm_cleaned_ratios)

summary_formalin_count_afdm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_formalin_count_afdm_outliers_removed_ratios) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_formalin_count_afdm <- bind_rows(summary_formalin_count_afdm_ratios, summary_formalin_count_afdm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_formalin_count_afdm,
          here("output/formalin/count_afdm/full_summary_formalin_count_afdm.csv"))
```
...
```{r}
pred_mass_formalin_count_afdm_outliers_removed_MAPE <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_afdm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_afdm = map2(.x = data,
                              .y = biomass_formalin_count_afdm_outliers_removed_ratios$mean_afdm_count,
...
pred_mass_formalin_count_afdm_outliers_removed_MAPE |> 
