...
```{r}
biomass_formalin_dm_afdm_ratios <- biomass_formalin_dm_afdm_cleaned |>
  mutate(afdm_dm = afdm_g/dm_g) |>
  group_nest(taxa_clean) |>
  mutate(afdm_dm_t_test = map(.x = data,
                            .f = ~ t.test(.x$afdm_dm)),
         ci_mean_afdm_dm = map(.x = afdm_dm_t_test,
                             .f = ~ .x$conf.int)) |>
  unnest(ci_mean_afdm_dm) |>
  mutate(ci_bound = c(rep(c("lower", "upper"), times = length(taxa_clean)/2))) |>
  pivot_wider(names_from = ci_bound, values_from = ci_mean_afdm_dm) |>
  unnest(data) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
            mean_afdm_dm = mean(afdm_dm),
            lower_ci_mean_afdm_dm = mean(lower),
            upper_ci_mean_afdm_dm = mean(upper),
...
write_csv(biomass_formalin_dm_afdm_ratios,
...
```{r}
pred_mass_formalin_dm_afdm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_afdm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_afdm = map2(.x = data,
                              .y = biomass_formalin_dm_afdm_ratios$mean_afdm_dm,
                              .f = ~ .y*.x$dm_g),
...
```{r}
pred_mass_formalin_dm_afdm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_afdm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$dm_g)^..3*..4),
         ols_pred_afdm_percent_error = map2(.x = data,
                                          .y = ols_pred_afdm,
...
```{r}
summary_formalin_dm_afdm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_formalin_dm_afdm_ratios) |>
  left_join(pred_mass_formalin_dm_afdm_error_eval)
...
summary_formalin_dm_afdm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
  ...
  left_join(pred_mass_formalin_dm_afdm_outliers_removed_error_eval) |>
