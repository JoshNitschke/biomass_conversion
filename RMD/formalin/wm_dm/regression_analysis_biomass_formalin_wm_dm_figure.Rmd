...
```{r}
biomass_formalin_wm_dm_cleaned <- read_csv(here("data/processed/formalin/wm_dm/biomass_formalin_wm_dm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_formalin_wm_dm_cleaned |> 
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_formalin_wm_dm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_formalin_wm_dm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                            .f = ~ lm(log(dm_g) ~ log(wm_g), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(wm_g), y = log(dm_g))) + 
                           geom_point() +
                           geom_smooth(method = "lm") +
                           labs(x = "log(e) wet mass (g)", y = "log(e) dry mass (g)") +
                           ggtitle(.y)))
...
write_csv(reg_summary_a_sf,
          here("output/formalin/wm_dm/biomass_regression_summary_formalin_wm_dm.csv"))
```
...
```{r}
biomass_formalin_wm_dm_ratios <- biomass_formalin_wm_dm_cleaned |>
...
write_csv(biomass_formalin_wm_dm_ratios,
          here("output/formalin/wm_dm/biomass_ratio_summary_formalin_wm_dm.csv"))
```
...
```{r}
pred_mass_formalin_wm_dm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_formalin_wm_dm_ratios$mean_dm_wm,
                              .f = ~ .y*.x$wm_g),
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_formalin_wm_dm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/formalin/wm_dm/biomass_regression_summary_formalin_wm_dm_outliers_removed.csv"))
```
...
taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI |>
  ...
  mutate(BT_plot = pmap(.l = list(data, slope, intercept, sf, taxa_clean, r2),
                        .f = ~ggplot(..1, aes(x = wm_g, y = dm_g)) +
                          geom_point(fill = "grey", size = 3, alpha = 0.5, stroke = 1) +
                          geom_function(fun = function(x) (exp(..3))*(x^..2)*..4, 
                                        col = "#D55E00", linewidth  = 1) +
                          geom_ribbon(aes(ymin = lower_CI_line, ymax = upper_CI_line), fill = "#D55E00", 
                                      alpha = 0.2) +
                          ggtitle(..5) + 
                          labs(x = "wet mass (g)", y = "dry mass (g)") + 
                          #geom_text(x = 0.25*max(..1$wm_g), y = 0.85*max(..1$dm_g), 
...
```{r}
biomass_formalin_wm_dm_outliers_removed_ratios <- biomass_formalin_wm_dm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(dm_wm = dm_g/wm_g) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_formalin_wm_dm_outliers_removed_ratios,
          here("output/formalin/wm_dm/biomass_ratio_summary_formalin_wm_dm_outliers_removed.csv"))
```
...
```{r}
pred_mass_formalin_wm_dm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$wm_g)^..3*..4),
         ols_pred_dm_percent_error = map2(.x = data,
                                          .y = ols_pred_dm,
                                          .f = ~-((.x$dm_g-.y)/.x$dm_g)*100),
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_formalin_wm_dm_outliers_removed_ratios$mean_dm_wm,
                              .f = ~ .y*.x$wm_g),
         ratio_pred_dm_percent_error = map2(.x = data,
                                          .y = ratio_pred_dm,
                                          .f = ~-((.x$dm_g-.y)/.x$dm_g)*100),
...
```{r}
summary_formalin_wm_dm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_formalin_wm_dm_ratios) |>
  left_join(pred_mass_formalin_wm_dm_error_eval)

summary_formalin_wm_dm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_formalin_wm_dm_outliers_removed_ratios) |>
  left_join(pred_mass_formalin_wm_dm_outliers_removed_error_eval) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_formalin_wm_dm <- bind_rows(summary_formalin_wm_dm_ratios, summary_formalin_wm_dm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_formalin_wm_dm,
          here("output/formalin/wm_dm/full_summary_formalin_wm_dm.csv"))
```

### Creating a multi-panel figure

Creating a multi-panel figure of back-transformed regression lines on raw data (outliers removed)

```{r}
margin = theme(plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))
png(here("output/png/formalin_wm_dm_ols_plots_outliers_removed.png"), width = 4400, height = 2800, res = 300)
par(omi = c(1,1,1,1))                    
grid.arrange(grobs = lapply(taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT$BT_plot, "+", margin), ncol = 3)
dev.off()
```