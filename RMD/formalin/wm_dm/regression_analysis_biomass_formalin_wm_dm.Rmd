...
biomass_formalin_wm_dm_ratios <- biomass_formalin_wm_dm_cleaned |>
  ...
  group_nest(taxa_clean) |>
  mutate(dm_wm_t_test = map(.x = data,
                            .f = ~ t.test(.x$dm_wm)),
         ci_mean_dm_wm = map(.x = dm_wm_t_test,
                             .f = ~ .x$conf.int)) |>
  unnest(ci_mean_dm_wm) |>
  mutate(ci_bound = c(rep(c("lower", "upper"), times = length(taxa_clean)/2))) |>
  pivot_wider(names_from = ci_bound, values_from = ci_mean_dm_wm) |>
  unnest(data) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
            mean_dm_wm = mean(dm_wm),
            lower_ci_mean_dm_wm = mean(lower),
            upper_ci_mean_dm_wm = mean(upper),
            sd_dm_wm = sd(dm_wm),
