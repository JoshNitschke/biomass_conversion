...
```{r}
biomass_formalin_wm_afdm_cleaned <- read_csv(here("data/processed/formalin/wm_afdm/biomass_formalin_wm_afdm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_formalin_wm_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_object = map(.x = data,
                          ~ lm(afdm_g ~ wm_g, data = .x)))
```
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_formalin_wm_afdm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_formalin_wm_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                            .f = ~ lm(log(afdm_g) ~ log(wm_g), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(wm_g), y = log(afdm_g))) + 
                           geom_point() +
                           geom_smooth(method = "lm") +
                           labs(x = "log(e) wet mass (g)", y = "log(e) ash-free dry mass (g)") +
                           ggtitle(.y)))
...
write_csv(reg_summary_a_sf,
          here("output/formalin/wm_afdm/biomass_regression_summary_formalin_wm_afdm.csv"))
```
...
taxa_nested_lm_ln_plot_coef_CI_BT <- taxa_nested_lm_ln_plot_coef_CI |>
  ...
  select(taxa_clean, wm_g, afdm_g, lower_CI_line, upper_CI_line, sf) |>
  nest(data = c(wm_g, afdm_g, lower_CI_line, upper_CI_line)) |>
  right_join(reg_summary_a) |>
  mutate(BT_plot = pmap(.l = list(data, slope, intercept, sf, taxa_clean, r2),
                        .f = ~ggplot(..1, aes(x = wm_g, y = afdm_g)) +
                          ...
                          geom_ribbon(aes(ymin = lower_CI_line, ymax = upper_CI_line), fill = "darkgreen", 
                                      alpha = 0.2) +
                          ggtitle(..5) + 
                          labs(x = "wet mass (g)", y = "ash-free dry mass (g)") + 
                          geom_text(x = 0.25*max(..1$wm_g), y = 0.85*max(..1$afdm_g), 
                                    label = deparse(bquote(paste("A = ", 
                                                                 ...
                                                                 sep = ""))),
                                    parse = TRUE, size = 4) +
                          geom_text(x = 0.25*max(..1$wm_g), y = 0.75*max(..1$afdm_g), 
...
```{r}
biomass_formalin_wm_afdm_cleaned_ratios <- biomass_formalin_wm_afdm_cleaned |>
  mutate(afdm_wm = afdm_g/wm_g) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
            mean_afdm_wm = mean(afdm_wm),
            sd_afdm_wm = sd(afdm_wm),
            lower_ci_mean_afdm_wm = mean_afdm_wm-0.95*(sd_afdm_wm/sqrt(n)),
            upper_ci_mean_afdm_wm = mean_afdm_wm+0.95*(sd_afdm_wm/sqrt(n)),
            geom_mean = prod(afdm_wm)^(1/n),
            min_afdm_wm = min(afdm_wm),
            median_afdm_wm = median(afdm_wm),
            max_afdm_wm = max(afdm_wm),
            range_wm = paste0(format(round(min(wm_g), digits = 3), nsmall = 3), 
                              "-", format(round(max(wm_g), digits = 3), 
                                          nsmall = 3)))

# Saving the ratio summary
write_csv(biomass_formalin_wm_afdm_cleaned_ratios,
          here("output/formalin/wm_afdm/biomass_ratio_summary_formalin_wm_afdm.csv"))
```
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_formalin_wm_afdm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                          ~ lm(log(afdm_g) ~ log(wm_g), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(wm_g), y = log(afdm_g))) + 
                           geom_point() +
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/formalin/wm_afdm/biomass_regression_summary_formalin_wm_afdm_outliers_removed.csv"))
```
...
```{r}
biomass_formalin_wm_afdm_outliers_removed_ratios <- biomass_formalin_wm_afdm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(afdm_wm = afdm_g/wm_g) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_formalin_wm_afdm_outliers_removed_ratios,
          here("output/formalin/wm_afdm/biomass_ratio_summary_formalin_wm_afdm_outliers_removed.csv"))
```

### combining summary tables

```{r}
summary_formalin_wm_afdm_ratios <- left_join(reg_summary_a_sf, biomass_formalin_wm_afdm_cleaned_ratios)

summary_formalin_wm_afdm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_formalin_wm_afdm_outliers_removed_ratios) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_formalin_wm_afdm <- bind_rows(summary_formalin_wm_afdm_ratios, summary_formalin_wm_afdm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_formalin_wm_afdm,
          here("output/formalin/wm_afdm/full_summary_formalin_wm_afdm.csv"))
```
...
```{r}
pred_mass_formalin_wm_afdm_outliers_removed_MAPE <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_afdm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_afdm = map2(.x = data,
                              .y = biomass_formalin_wm_afdm_outliers_removed_ratios$mean_afdm_wm,
                              .f = ~ .y*.x$wm_g),
...
pred_mass_formalin_wm_afdm_outliers_removed_MAPE |> 
