...
### Pull out regression summary statistics
...
### Calculating SF (smearing factor) (mean of back-transformed residuals)
...
taxa_nested_sma_log10_plot_coef_BT <- taxa_nested_sma_log10_plot_coef_sf |> 
  select(taxa_clean, data, summary, sf) |> 
  unnest(summary) |>
  mutate(BT_plot = pmap(.l = list(data, Slope, Int, sf, taxa_clean),
                        .f = ~ggplot(..1, aes(x = wm_g, y = dm_g)) +
                          geom_point() +
                          geom_function(fun = function(x) (10^..3)*x^..2) +
                          geom_function(fun = function(x) (10^..3)*(x^..2)*..4, 
                                        col = "green") +
                          ggtitle(..5)))
