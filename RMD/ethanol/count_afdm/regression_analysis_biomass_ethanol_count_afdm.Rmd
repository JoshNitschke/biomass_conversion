...
```{r}
taxa_nested_sma <- biomass_ethanol_count_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(sma_object = map(.x = data,
                          ~ sma(afdm_g ~ individual_count, data = .x)))
```
...
```{r}
taxa_nested_sma_outliers_removed <- biomass_ethanol_count_afdm_cleaned |> 
...
```{r}
taxa_nested_power_analysis_check <- biomass_ethanol_count_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(power_analysis_test = map(.x = data,
                          ~ power_analysis(.x$individual_count, .x$afdm_g)))
```
...
```{r}
taxa_nested_sma_log10_plot <- biomass_ethanol_count_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(sma_log10_object = map(.x = data,
                          ~ sma(afdm_g ~ individual_count, log = "xy", data = .x)),
         sma_log10_plots = map2(.x = sma_log10_object,
                         .y = taxa_clean,
                         .f = ~ plot(.x, xlab = "individual count", ylab = "ash-free dry mass (g)", main = .y)))
...
taxa_nested_sma_log10_plot_coef_BT <- taxa_nested_sma_log10_plot_coef_sf |> 
  ...
  nest(data = c(individual_count, afdm_g, lower_CI_line, upper_CI_line, keep_data)) |>
  ...
  mutate(BT_plot = pmap(.l = list(data, Slope, Slope_lowCI, Slope_highCI, Int, Int_lowCI, Int_highCI, sf,
                        ...
                        .f = ~ggplot(..1, aes(x = individual_count, y = afdm_g)) +
                          ...
                          geom_text(x = 0.25*max(..1$individual_count), y = 0.85*max(..1$afdm_g), 
                                    label = deparse(bquote(paste("A = ", .(round((10^..5), digits = 2)), 
                                                                 "C" ^.(round(..2, digits = 2)),
...
```{r}
taxa_nested_outliers_removed_sma_log10_plot <- biomass_ethanol_count_afdm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(sma_log10_object = map(.x = data,
                          ~ sma(afdm_g ~ individual_count, log = "xy", data = .x)),
