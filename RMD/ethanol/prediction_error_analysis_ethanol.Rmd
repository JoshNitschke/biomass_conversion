...
total_percentage_error_plot_ethanol_wm_dm <- 
  ggplot(data = pred_mass_ethanol_wm_dm_error_long, 
         aes(x = pred_method, y = percent_error)) +
  ...
  scale_x_discrete(limits = c("reg_pred_dm_percent_error",  
                              "ratio_pred_dm_percent_error",
    ...
    scale_colour_manual(values = brewer.pal(8, "Dark2"), 
                        labels = c("Amphipoda", 
                                   substitute(paste(italic("Arthritica semen"))),
                                   substitute(paste(italic("Capitella"), "spp.")),
                                   "Chironomidae", 
                                   substitute(paste(italic("Salinator"), "spp.")),
                                   substitute(paste(italic("Simplisetia aequisetis"))),
                                   substitute(paste(italic("Spisula trigonella"))),
                                   "Truncatelloidea")) +
...
total_percentage_error_plot_ethanol_dm_afdm <- 
  ggplot(data = pred_mass_ethanol_dm_afdm_error_long, 
         aes(x = pred_method, y = percent_error)) +
  geom_jitter(width = 0.1, size = 3, aes(colour = taxa_clean), alpha = 0.5, stroke = 1) + 
  geom_hline(yintercept = 0, linetype = 2) +
  ggtitle(expression("Dry mass" %->% "Ash-free dry mass")) +
  labs(x = "Prediction method", y = "Percentage error in total mass") +
  scale_x_discrete(limits = c("reg_pred_afdm_percent_error", 
                              "ratio_pred_afdm_percent_error",
                              "riccbourg_ratio_pred_afdm_percent_error",
                              "breyetal_ratio_pred_afdm_percent_error"), 
                   labels = c("Log-linear regression \n(this study)", 
                              "Mean ratio \n(this study)", 
                              "Ricciardi and Bourget \nmean ratio", 
...
```{r}
png(here("output/png/total_percentage_error_plot_ethanol_combined.png"), width = 190, height = 250, units = "mm", res = 1000)
par(omi = c(1,1,1,1))  
