...
```{r}
biomass_ethanol_wm_afdm_cleaned <- read_csv(here("data/processed/ethanol/wm_afdm/biomass_ethanol_wm_afdm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_ethanol_wm_afdm_cleaned |> 
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_ethanol_wm_afdm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_ethanol_wm_afdm_cleaned |> 
  group_nest(taxa_clean) |>
...
write_csv(reg_summary_a_sf,
          here("output/ethanol/wm_afdm/biomass_regression_summary_ethanol_wm_afdm.csv"))
```
...
```{r}
biomass_ethanol_wm_afdm_cleaned_ratios <- biomass_ethanol_wm_afdm_cleaned |>
...
write_csv(biomass_ethanol_wm_afdm_cleaned_ratios,
          here("output/ethanol/wm_afdm/biomass_ratio_summary_ethanol_wm_afdm.csv"))
```
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_ethanol_wm_afdm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/ethanol/wm_afdm/biomass_regression_summary_ethanol_wm_afdm_outliers_removed.csv"))
```
...
```{r}
biomass_ethanol_wm_afdm_outliers_removed_ratios <- biomass_ethanol_wm_afdm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(afdm_wm = afdm_g/wm_g) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_ethanol_wm_afdm_outliers_removed_ratios,
          here("output/ethanol/wm_afdm/biomass_ratio_summary_ethanol_wm_afdm_outliers_removed.csv"))
```

### combining summary tables

```{r}
summary_ethanol_wm_afdm_ratios <- left_join(reg_summary_a_sf, biomass_ethanol_wm_afdm_cleaned_ratios)

summary_ethanol_wm_afdm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_ethanol_wm_afdm_outliers_removed_ratios) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_ethanol_wm_afdm <- bind_rows(summary_ethanol_wm_afdm_ratios, summary_ethanol_wm_afdm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_ethanol_wm_afdm,
          here("output/ethanol/wm_afdm/full_summary_ethanol_wm_afdm.csv"))
```
...
```{r}
pred_mass_ethanol_wm_afdm_outliers_removed_MAPE <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_afdm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_afdm = map2(.x = data,
                              .y = biomass_ethanol_wm_afdm_outliers_removed_ratios$mean_afdm_wm,
                              .f = ~ .y*.x$wm_g),
...
pred_mass_ethanol_wm_afdm_outliers_removed_MAPE |> 
