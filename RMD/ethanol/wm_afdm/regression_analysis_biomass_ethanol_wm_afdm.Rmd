...
```{r}
biomass_ethanol_wm_afdm_ratios <- biomass_ethanol_wm_afdm_cleaned |>
...
write_csv(biomass_ethanol_wm_afdm_ratios,
...
```{r}
pred_mass_ethanol_wm_afdm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_afdm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_afdm = map2(.x = data,
                              .y = biomass_ethanol_wm_afdm_ratios$mean_afdm_wm,
                              .f = ~ .y*.x$wm_g),
...
```{r}
pred_mass_ethanol_wm_afdm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_afdm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$wm_g)^..3*..4),
         ols_pred_afdm_percent_error = map2(.x = data,
                                          .y = ols_pred_afdm,
...
```{r}
summary_ethanol_wm_afdm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_ethanol_wm_afdm_ratios) |>
  left_join(pred_mass_ethanol_wm_afdm_error_eval)
...
summary_ethanol_wm_afdm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
  ...
  left_join(pred_mass_ethanol_wm_afdm_outliers_removed_error_eval) |>
