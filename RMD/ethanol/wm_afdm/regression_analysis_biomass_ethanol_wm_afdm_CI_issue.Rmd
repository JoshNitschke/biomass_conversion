...
```{r}
taxa_nested_sma <- biomass_ethanol_wm_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(sma_object = map(.x = data,
                          ~ sma(afdm_g ~ wm_g, data = .x)))
```
...
```{r}
taxa_nested_sma_outliers_removed <- biomass_ethanol_wm_afdm_cleaned |> 
...
```{r}
taxa_nested_power_analysis_check <- biomass_ethanol_wm_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(power_analysis_test = map(.x = data,
                          ~ power_analysis(.x$wm_g, .x$afdm_g)))
```
...
```{r}
taxa_nested_sma_log10_plot <- biomass_ethanol_wm_afdm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(sma_log10_object = map(.x = data,
                          ~ sma(afdm_g ~ wm_g, log = "xy", data = .x)),
         sma_log10_plots = map2(.x = sma_log10_object,
                         .y = taxa_clean,
                         .f = ~ plot(.x, xlab = "wet mass (g)", ylab = "ash-free dry mass (g)", main = .y)))
...
taxa_nested_sma_log10_plot_coef_BT <- taxa_nested_sma_log10_plot_coef_sf |> 
  ...
  nest(data = c(wm_g, afdm_g, lower_CI_line, upper_CI_line, keep_data)) |>
...
```{r}
taxa_nested_outliers_removed_sma_log10_plot <- biomass_ethanol_wm_afdm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(sma_log10_object = map(.x = data,
                          ~ sma(afdm_g ~ wm_g, log = "xy", data = .x)),
...
```{r}
biomass_ethanol_wm_afdm_cleaned_ln_sma <- biomass_ethanol_wm_afdm_cleaned |>
  mutate(wm_mg = wm_g*1000,
         afdm_mg = afdm_g*1000,
         ln_wm_mg = log(wm_mg),
         ln_afdm_mg = log(afdm_mg)) |>
  group_nest(taxa_clean) |>
  mutate(sma_ln_object = map(.x = data,
                          ~ sma(ln_afdm_mg ~ ln_wm_mg, data = .x)),
         sma_ln_plots = map2(.x = sma_ln_object,
                         .y = taxa_clean,
                         .f = ~ plot(.x, xlab = "wet mass (mg)", ylab = "ash-free dry mass (mg)", main = .y)))


taxa_nested_sma_ln_plot_coef <- biomass_ethanol_wm_afdm_cleaned_ln_sma |>
...
taxa_nested_sma_ln_plot_coef_BT <- taxa_nested_sma_ln_plot_coef_sf |> 
  ...
  nest(data = c(wm_g, afdm_g, wm_mg, afdm_mg, ln_wm_mg, ln_afdm_mg, lower_CI_line, upper_CI_line, keep_data)) |>
  select(taxa_clean, data, Slope, Slope_lowCI, Slope_highCI, Int, Int_lowCI, Int_highCI, sf, r2) |>
  mutate(BT_plot = pmap(.l = list(data, Slope, Slope_lowCI, Slope_highCI, Int, Int_lowCI, Int_highCI, sf,
                                  taxa_clean, r2),
                        .f = ~ggplot(..1, aes(x = wm_mg, y = afdm_mg)) +
                          geom_point(fill = "grey", size = 3, alpha = 0.5, stroke = 1) +
                          ...
                          geom_ribbon(aes(ymin = lower_CI_line, ymax = upper_CI_line), fill = "darkgreen", 
                                      alpha = 0.2) +
                          ggtitle(..9) + 
                          labs(x = "wet mass (mg)", y = "ash-free dry mass (mg)") + 
                          geom_text(x = 0.25*max(..1$wm_mg), y = 0.85*max(..1$afdm_mg), 
