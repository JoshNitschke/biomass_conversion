...
```{r}
biomass_ethanol_count_dm_ratios <- biomass_ethanol_count_dm_cleaned |>
...
write_csv(biomass_ethanol_count_dm_ratios,
...
```{r}
pred_mass_ethanol_count_dm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_ethanol_count_dm_ratios$mean_dm_count,
...
```{r}
pred_mass_ethanol_count_dm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$individual_count)^..3*..4),
         ols_pred_dm_percent_error = map2(.x = data,
                                          .y = ols_pred_dm,
...
```{r}
summary_ethanol_count_dm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_ethanol_count_dm_ratios) |>
  left_join(pred_mass_ethanol_count_dm_error_eval)
...
summary_ethanol_count_dm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
  ...
  left_join(pred_mass_ethanol_count_dm_outliers_removed_error_eval) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_ethanol_count_dm <- bind_rows(summary_ethanol_count_dm_ratios, summary_ethanol_count_dm_ratios_outliers_removed) |>
  arrange(taxa_clean)
