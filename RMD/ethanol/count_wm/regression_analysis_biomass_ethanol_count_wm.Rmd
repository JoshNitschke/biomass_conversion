...
```{r}
taxa_nested_lm <- biomass_ethanol_count_wm_cleaned |> 
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_ethanol_count_wm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_ethanol_count_wm_cleaned |> 
  group_nest(taxa_clean) |>
...
write_csv(reg_summary_a_sf,
          here("output/ethanol/count_wm/biomass_regression_summary_ethanol_count_wm.csv"))
```
...
write_csv(biomass_ethanol_count_wm_cleaned_ratios,
          here("output/ethanol/count_wm/biomass_ratio_summary_ethanol_count_wm.csv"))
```
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_ethanol_count_wm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/ethanol/count_wm/biomass_regression_summary_ethanol_count_wm_outliers_removed.csv"))
```
...
```{r}
biomass_ethanol_count_wm_outliers_removed_ratios <- biomass_ethanol_count_wm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(wm_count = wm_g/individual_count) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_ethanol_count_wm_outliers_removed_ratios,
          here("output/ethanol/count_wm/biomass_ratio_summary_ethanol_count_wm_outliers_removed.csv"))
```

### combining summary tables

```{r}
summary_ethanol_count_wm_ratios <- left_join(reg_summary_a_sf, biomass_ethanol_count_wm_cleaned_ratios)

summary_ethanol_count_wm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_ethanol_count_wm_outliers_removed_ratios) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_ethanol_count_wm <- bind_rows(summary_ethanol_count_wm_ratios, summary_ethanol_count_wm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_ethanol_count_wm,
          here("output/ethanol/count_wm/full_summary_ethanol_count_wm.csv"))
```
...
```{r}
pred_mass_ethanol_count_wm_outliers_removed_MAPE <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_wm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_wm = map2(.x = data,
                              .y = biomass_ethanol_count_wm_outliers_removed_ratios$mean_wm_count,
...
pred_mass_ethanol_count_wm_outliers_removed_MAPE |> 
