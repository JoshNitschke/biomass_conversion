---
title: "Cleaning biomass data - individual count and wet mass"
...
```{r}
step_1 <- min_cutoff(data = biomass_data_taxaclean, individual_count, wm_g) 
```
...
```{r}
biomass_ethanol_percentiles <- step_3 |>
  group_by(taxa_clean) |>
...
taxa_nested <- biomass_ethanol_percentiles |> 
  select(taxa_clean, individual_count, wm_g, obs_num, keep_data_ind_count_percentiles, keep_data_wm_percentiles) |> 
  nest(data = c(individual_count, wm_g, obs_num, keep_data_ind_count_percentiles, keep_data_wm_percentiles))
...
taxa_nested_outlier_candidates_identified <- taxa_nested |> 
  mutate(keep_data_leverage = map(.x = data,
                                  ~ keep_data_leverage(.x, "individual_count", "wm_g")),
         keep_data_stud_resid = map(.x = data,
                                    ~ keep_data_stud_resid(.x, "individual_count", "wm_g")),
         keep_data_cook = map(.x = data,
                              ~ keep_data_cook(.x, "individual_count", "wm_g")),
         keep_data_dffits = map(.x = data,
                                ~ keep_data_dffits(.x, "individual_count", "wm_g")),
         keep_data_dfbetas = map(.x = data,
                                ~ keep_data_dfbetas(.x, "individual_count", "wm_g")))
```
...
taxa_unnested_keep_data_merged <- taxa_unnested_outlier_candidates_identified |>
 ...
 # Consolidating all outlier candidates into one vector
   mutate(keep_data = ifelse(keep_data_ind_count_percentiles == TRUE & keep_data_wm_percentiles == TRUE & keep_data_cook == TRUE & keep_data_leverage == TRUE & keep_data_stud_resid == TRUE & keep_data_dffits == TRUE & keep_data_dfbetas_int == TRUE & keep_data_dfbetas_slope == TRUE, TRUE, FALSE)) 
```
...
## Saving the data
```{r}
biomass_ethanol_count_wm_cleaned <- taxa_unnested_keep_data_merged |>
  select(taxa_clean, individual_count, wm_g, keep_data)

write_csv(biomass_ethanol_count_wm_cleaned,
          here("data/processed/ethanol/count_wm/biomass_ethanol_count_wm_cleaned.csv"))
```
