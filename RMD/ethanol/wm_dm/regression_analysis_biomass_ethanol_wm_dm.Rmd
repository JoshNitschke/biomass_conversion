...
```{r}
taxa_nested_lm <- biomass_ethanol_wm_dm_cleaned |> 
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_ethanol_wm_dm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_ethanol_wm_dm_cleaned |> 
  group_nest(taxa_clean) |>
...
write_csv(reg_summary_a_sf,
          here("output/ethanol/wm_dm/biomass_regression_summary_ethanol_wm_dm.csv"))
```
...
write_csv(biomass_ethanol_wm_dm_cleaned_ratios,
          here("output/ethanol/wm_dm/biomass_ratio_summary_ethanol_wm_dm.csv"))
```
...
```{r}
biomass_ethanol_wm_dm_outliers_removed_ratios <- biomass_ethanol_wm_dm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(dm_wm = dm_g/wm_g) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
...
# Saving the ratio summary
write_csv(biomass_ethanol_wm_dm_outliers_removed_ratios,
          here("output/ethanol/wm_dm/biomass_ratio_summary_ethanol_wm_dm_outliers_removed.csv"))
```

### combining summary tables

```{r}
summary_ethanol_wm_dm_ratios <- left_join(reg_summary_a_sf, biomass_ethanol_wm_dm_cleaned_ratios)

summary_ethanol_wm_dm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_ethanol_wm_dm_outliers_removed_ratios) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_ethanol_wm_dm <- bind_rows(summary_ethanol_wm_dm_ratios, 
                                        summary_ethanol_wm_dm_ratios_outliers_removed) |>
...
write_csv(full_summary_ethanol_wm_dm,
          here("output/ethanol/wm_dm/full_summary_ethanol_wm_dm.csv"))
```
...
```{r}
pred_mass_ethanol_wm_dm_outliers_removed_MAPE <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_ethanol_wm_dm_outliers_removed_ratios$mean_dm_wm,
                              .f = ~ .y*.x$wm_g),
...
pred_mass_ethanol_wm_dm_outliers_removed_MAPE |> 
