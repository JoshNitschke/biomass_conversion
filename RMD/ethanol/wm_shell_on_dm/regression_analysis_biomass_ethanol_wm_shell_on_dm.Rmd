...
```{r}
biomass_ethanol_wm_shell_on_dm_cleaned <- read_csv(here("data/processed/ethanol/wm_shell_on_dm/biomass_ethanol_wm_shell_on_dm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_ethanol_wm_shell_on_dm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_object = map(.x = data,
                          ~ lm(dm_g ~ wm_shell_on_g, data = .x)))
```
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_ethanol_wm_shell_on_dm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_ethanol_wm_shell_on_dm_cleaned |> 
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                            .f = ~ lm(log(dm_g) ~ log(wm_shell_on_g), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(wm_shell_on_g), y = log(dm_g))) + 
                           geom_point() +
                           geom_smooth(method = "lm") +
                           labs(x = "log(e) wet mass (shell on) (g)", y = "log(e) dry mass (g)") +
                           ggtitle(.y)))
...
write_csv(reg_summary_a_sf,
          here("output/ethanol/wm_shell_on_dm/biomass_regression_summary_ethanol_wm_shell_on_dm.csv"))
```
...
taxa_nested_lm_ln_plot_coef_CI_BT <- taxa_nested_lm_ln_plot_coef_CI |>
  ...
  select(taxa_clean, wm_shell_on_g, dm_g, lower_CI_line, upper_CI_line, sf) |>
  nest(data = c(wm_shell_on_g, dm_g, lower_CI_line, upper_CI_line)) |>
  right_join(reg_summary_a) |>
  mutate(BT_plot = pmap(.l = list(data, slope, intercept, sf, taxa_clean, r2),
                        .f = ~ggplot(..1, aes(x = wm_shell_on_g, y = dm_g)) +
                          ...
                          geom_ribbon(aes(ymin = lower_CI_line, ymax = upper_CI_line), fill = "darkgreen", 
                                      alpha = 0.2) +
                          ggtitle(..5) + 
                          labs(x = "wet mass (shell on) (g)", y = "dry mass (g)") + 
                          geom_text(x = 0.25*max(..1$wm_shell_on_g), y = 0.85*max(..1$dm_g), 
                                    label = deparse(bquote(paste("D = ", 
                                                                 ...
                                                                 sep = ""))),
                                    parse = TRUE, size = 4) +
                          geom_text(x = 0.25*max(..1$wm_shell_on_g), y = 0.75*max(..1$dm_g), 
...
```{r}
biomass_ethanol_wm_shell_on_dm_ratios <- biomass_ethanol_wm_shell_on_dm_cleaned |>
  mutate(dm_wm_shell_on = dm_g/wm_shell_on_g) |>
  group_nest(taxa_clean) |>
  mutate(dm_wm_shell_on_t_test = map(.x = data,
                            .f = ~ t.test(.x$dm_wm_shell_on)),
         ci_mean_dm_wm_shell_on = map(.x = dm_wm_shell_on_t_test,
                             .f = ~ .x$conf.int)) |>
  unnest(ci_mean_dm_wm_shell_on) |>
  mutate(ci_bound = c(rep(c("lower", "upper"), times = length(taxa_clean)/2))) |>
  pivot_wider(names_from = ci_bound, values_from = ci_mean_dm_wm_shell_on) |>
  unnest(data) |>
  group_by(taxa_clean) |>
  summarise(n = n(),
            mean_dm_wm_shell_on = mean(dm_wm_shell_on),
            lower_ci_mean_dm_wm_shell_on = mean(lower),
            upper_ci_mean_dm_wm_shell_on = mean(upper),
            sd_dm_wm_shell_on = sd(dm_wm_shell_on),
            geom_mean = prod(dm_wm_shell_on)^(1/n),
            min_dm_wm_shell_on = min(dm_wm_shell_on),
            median_dm_wm_shell_on = median(dm_wm_shell_on),
            max_dm_wm_shell_on = max(dm_wm_shell_on),
...
write_csv(biomass_ethanol_wm_shell_on_dm_ratios,
          here("output/ethanol/wm_shell_on_dm/biomass_ratio_summary_ethanol_wm_shell_on_dm.csv"))
```
...
```{r}
pred_mass_ethanol_wm_shell_on_dm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_ethanol_wm_shell_on_dm_ratios$mean_dm_wm_shell_on,
                              .f = ~ .y*.x$wm_shell_on_g),
         ...
         ols_total_PE = map2(.x = data,
                             .y = ols_pred_dm,
                             .f = ~ (sum(.y-.x$dm_g)/sum(.x$dm_g))*100),
         ols_MPE = map2(.x = ols_pred_dm_percent_error,
                        .y = n,
                        .f = ~ sum(.x)/.y),
         ols_MAPE = map2(.x = ols_pred_dm_percent_error,
                       .y = n,
                       .f = ~ sum(abs(.x))/.y),
         ratio_total_PE = map2(.x = data,
                             .y = ratio_pred_dm,
                             .f = ~ (sum(.y-.x$dm_g)/sum(.x$dm_g))*100),
         ratio_MPE = map2(.x = ratio_pred_dm_percent_error,
                        .y = n,
                        .f = ~ sum(.x)/.y),
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_ethanol_wm_shell_on_dm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
                          ~ lm(log(dm_g) ~ log(wm_shell_on_g), data = .x)),
         lm_ln_plots = map2(.x = data,
                         .y = taxa_clean,
                         .f = ~ ggplot(.x, aes(x = log(wm_shell_on_g), y = log(dm_g))) + 
                           geom_point() +
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/ethanol/wm_shell_on_dm/biomass_regression_summary_ethanol_wm_shell_on_dm_outliers_removed.csv"))
```
...
```{r}
biomass_ethanol_wm_shell_on_dm_outliers_removed_ratios <- biomass_ethanol_wm_shell_on_dm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(dm_wm_shell_on = dm_g/wm_shell_on_g) |>
  group_nest(taxa_clean) |>
  mutate(dm_wm_shell_on_t_test = map(.x = data,
...
# Saving the ratio summary
write_csv(biomass_ethanol_wm_shell_on_dm_outliers_removed_ratios,
          here("output/ethanol/wm_shell_on_dm/biomass_ratio_summary_ethanol_wm_shell_on_dm_outliers_removed.csv"))
```
...
```{r}
pred_mass_ethanol_wm_shell_on_dm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$wm_shell_on_g)^..3*..4),
         ols_pred_dm_percent_error = map2(.x = data,
                                          .y = ols_pred_dm,
                                          .f = ~-((.x$dm_g-.y)/.x$dm_g)*100),
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_ethanol_wm_shell_on_dm_outliers_removed_ratios$mean_dm_wm_shell_on,
                              .f = ~ .y*.x$wm_shell_on_g),
         ratio_pred_dm_percent_error = map2(.x = data,
                                          .y = ratio_pred_dm,
                                          .f = ~-((.x$dm_g-.y)/.x$dm_g)*100),
...
```{r}
summary_ethanol_wm_shell_on_dm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_ethanol_wm_shell_on_dm_ratios) |>
  left_join(pred_mass_ethanol_wm_shell_on_dm_error_eval)

summary_ethanol_wm_shell_on_dm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_ethanol_wm_shell_on_dm_outliers_removed_ratios) |>
  left_join(pred_mass_ethanol_wm_shell_on_dm_outliers_removed_error_eval) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_ethanol_wm_shell_on_dm <- bind_rows(summary_ethanol_wm_shell_on_dm_ratios, summary_ethanol_wm_shell_on_dm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_ethanol_wm_shell_on_dm,
          here("output/ethanol/wm_shell_on_dm/full_summary_ethanol_wm_shell_on_dm.csv"))
```