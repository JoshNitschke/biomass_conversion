...
```{r}
step_1 <- min_cutoff(data = biomass_data_taxaclean, wm_shell_on_g, dm_g)  
```

Removing observations when wet mass is less than dry mass (considering these errors in measurement or recording)

```{r}
step_2 <- check_mass_diff(step_1, wm_shell_on_g, dm_g)
```
...
taxa_nested <- biomass_ethanol_percentiles |> 
  select(taxa_clean, wm_shell_on_g, dm_g, obs_num, keep_data_wm_shell_on_percentiles, keep_data_dm_percentiles) |> 
  nest(data = c(wm_shell_on_g, dm_g, obs_num, keep_data_wm_shell_on_percentiles, keep_data_dm_percentiles))
...
taxa_nested_outlier_candidates_identified <- taxa_nested |> 
  mutate(keep_data_leverage = map(.x = data,
                                  ~ keep_data_leverage(.x, "wm_shell_on_g", "dm_g")),
         keep_data_stud_resid = map(.x = data,
                                    ~ keep_data_stud_resid(.x, "wm_shell_on_g", "dm_g")),
         keep_data_cook = map(.x = data,
                              ~ keep_data_cook(.x, "wm_shell_on_g", "dm_g")),
         keep_data_dffits = map(.x = data,
                                ~ keep_data_dffits(.x, "wm_shell_on_g", "dm_g")),
         keep_data_dfbetas = map(.x = data,
                                ~ keep_data_dfbetas(.x, "wm_shell_on_g", "dm_g")))
```
...
taxa_unnested_keep_data_merged <- taxa_unnested_outlier_candidates_identified |>
 ...
 # Consolidating all outlier candidates into one vector
   mutate(keep_data = ifelse(keep_data_wm_shell_on_percentiles == TRUE & keep_data_dm_percentiles == TRUE & keep_data_cook == TRUE & keep_data_leverage == TRUE & keep_data_stud_resid == TRUE & keep_data_dffits == TRUE & keep_data_dfbetas_int == TRUE & keep_data_dfbetas_slope == TRUE, TRUE, FALSE)) 
```
...
## Saving the data
```{r}
biomass_ethanol_wm_shell_on_dm_cleaned <- taxa_unnested_keep_data_merged |>
  select(taxa_clean, wm_shell_on_g, dm_g, keep_data)

write_csv(biomass_ethanol_wm_shell_on_dm_cleaned, here("data/processed/ethanol/wm_shell_on_dm/biomass_ethanol_wm_shell_on_dm_cleaned.csv"))
```
