...
## Calculating R-squared on the original scale

```{r}

pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_SS <- pred_mass_ethanol_dm_afdm_outliers_removed_MAPE |>
  mutate(RSS = map2(.x = data,
                    .y = ols_pred_afdm,
                    .f = ~sum((.x$afdm_g-.y)^2)),
         TSS = map(.x = data,
                    .f = ~sum((.x$afdm_g-mean(.x$afdm_g))^2)))
pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_SS_original_scale_r2 <- pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_SS |>
  select(taxa_clean, RSS, TSS, r2) |> 
  unnest(c(RSS, TSS, r2)) |>
  mutate(original_r2 = 1-(RSS/TSS))

pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_original_r2 <- pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_SS |>
  unnest(c(RSS, TSS))
pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_SS_unnest <- pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_SS |>
  select(taxa_clean, RSS, TSS) |> unnest(c(RSS, TSS))
pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_SS_unnest |>
  mutate(original_r2 = map2(.x = RSS,
                            .y = TSS,
                            .f = ~1-(RSS/TSS)))
pred_mass_ethanol_dm_afdm_outliers_removed_MAPE_original_r2$original_r2
```