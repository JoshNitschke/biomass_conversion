...
taxa_nested_sma_ln_coef_ethanol_dm_afdm <- biomass_ethanol_dm_afdm_cleaned |>
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(sma_ln_object = map(.x = data,
                         .f = ~ sma(log(afdm_g) ~ log(dm_g), data = .x)),
         summary = map(.x = sma_ln_object,
                       .f = ~ (smatr:::summary.sma(.x))))

sma_summary_ethanol_dm_afdm <- taxa_nested_sma_ln_coef_ethanol_dm_afdm |> 
...
sma_summary_ethanol_dm_afdm_a <- sma_summary_ethanol_dm_afdm |> 
  mutate(a = exp(Int))
```
...
```{r}
taxa_nested_sma_ln_coef_sf_ethanol_dm_afdm <- taxa_nested_sma_ln_coef_ethanol_dm_afdm |>
  mutate(sf = map(.x = sma_ln_object,
                  .f = ~ (1/length(residuals(.x)))*sum(exp(residuals(.x)))))

taxa_sf_sma_ethanol_dm_afdm <- taxa_nested_sma_ln_coef_sf_ethanol_dm_afdm |>
  select(taxa_clean, sf) |>
  unnest(sf)

# Adding smearing factor to SMA summary table
sma_summary_ethanol_dm_afdm_a_sf <- merge(sma_summary_ethanol_dm_afdm_a,
                                            taxa_sf_sma_ethanol_dm_afdm)

# Adding suffix to variable names for later identification
names(sma_summary_ethanol_dm_afdm_a_sf) <- paste0(names(sma_summary_ethanol_dm_afdm_a_sf), "_sma")
sma_summary_ethanol_dm_afdm_a_sf <- rename(sma_summary_ethanol_dm_afdm_a_sf, "taxa_clean" = "taxa_clean_sma") |>
  clean_names()
```
...
taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI |>
  ...
  right_join(sma_summary_ethanol_dm_afdm_a_sf) |>
  mutate(BT_plot = pmap(.l = list(data, slope, intercept, sf, taxa_clean, r2, slope_sma, a_sma, sf_sma),
                        .f = ~ggplot(..1, aes(x = dm_g, y = afdm_g)) +
                          ...
                          geom_function(fun = function(x) mean(..1$afdm_g/..1$dm_g)*x,
...
#png(here("output/png/ethanol_dm_afdm_ols_ratio_comparison.png"), width = 2000, height = 1400, res = 300)
#par(omi = c(1,1,1,1))  
...
Brey, T., C. Müller-Wiegmann, Z. M. C. Zittier, and W. Hagen. 2010. Body composition in aquatic organisms — A global data bank of relationships between mass, elemental composition and energy content. Journal of Sea Research 64:334-340.
```{r}
...
taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT_riccbourg_brey <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI |>
  add_column(lower_CI, upper_CI) |>
  mutate(lower_CI_line = exp(lower_CI)*sf,
         upper_CI_line = exp(upper_CI)*sf) |>
  select(taxa_clean, dm_g, afdm_g, lower_CI_line, upper_CI_line, sf) |>
  ...
  mutate(BT_plot = pmap(.l = list(data, slope, intercept, sf, taxa_clean, r2, slope_sma, a_sma, sf_sma, ricc_bourg_mean_ratio, brey_mean_ratio),
                        .f = ~ggplot(..1, aes(x = dm_g, y = afdm_g)) +
                          geom_point(fill = "grey", size = 3, alpha = 0.5, stroke = 1) +
                          geom_function(fun = function(x) (exp(..3))*(x^..2)*..4, 
                                        col = "#009E73", linewidth  = 1) +
                          #geom_function(fun = function(x) ..8*x^..7*..9,
                          #              col = "orange", linewidth  = 1) +
                          geom_function(fun = function(x) mean(..1$afdm_g/..1$dm_g)*x,
                                        col = "#009E73", linewidth  = 1, linetype = 2) +
                          geom_function(fun = function(x) ..10*x,
                                        col = "#CC79A7", linewidth  = 1, linetype = 2) +
                          geom_function(fun = function(x) ..11*x,
                                        col = "#D55E00", linewidth  = 1, linetype = 2) +
                         # geom_ribbon(aes(ymin = lower_CI_line, ymax = upper_CI_line),
                         # fill = "#009E73", alpha = 0.2) +
                          ...
                          scale_x_continuous(expand = expansion(mult = c(0, 0.1)), limits = c(0, NA)) + 
                          scale_y_continuous(expand = expansion(mult = c(0, 0.1)), limits = c(0, NA))))
taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT_riccbourg_brey$BT_plot
```
...
#png(here("output/png/ethanol_dm_afdm_ols_ratio_comparison.png"), width = 2000, height = 1400, res = 300)
#par(omi = c(1,1,1,1))  
#taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT_riccbourg_brey$BT_plot[3]
#dev.off()
