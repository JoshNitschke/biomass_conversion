---
title: "Cleaning biomass data - dry mass and ash-free dry mass"
...
biomass_ethanol_percentiles <- step_4 |>
  group_by(taxa_clean) |>
  mutate(keep_data_dm_percentiles = percentiles(dm_g, 0.025, 0.975),
...
biomass_ethanol_percentiles |> 
  filter(keep_data_dm_percentiles == FALSE) |> 
  select(taxa_clean, dm_g, keep_data_dm_percentiles, obs_num) |>
  arrange(taxa_clean, dm_g) |>
  print(n = Inf)
```
...
taxa_nested <- biomass_ethanol_percentiles |> 
  select(taxa_clean, dm_g, afdm_g, obs_num, keep_data_dm_percentiles, keep_data_afdm_percentiles) |> 
  nest(data = c(dm_g, afdm_g, obs_num, keep_data_dm_percentiles, keep_data_afdm_percentiles))
...
taxa_nested_outlier_candidates_identified <- taxa_nested |> 
  mutate(keep_data_leverage = map(.x = data,
                                  ~ keep_data_leverage(.x, "dm_g", "afdm_g")),
         keep_data_stud_resid = map(.x = data,
                                    ~ keep_data_stud_resid(.x, "dm_g", "afdm_g")),
         keep_data_cook = map(.x = data,
                              ~ keep_data_cook(.x, "dm_g", "afdm_g")),
         keep_data_dffits = map(.x = data,
                                ~ keep_data_dffits(.x, "dm_g", "afdm_g")),
         keep_data_dfbetas = map(.x = data,
                                ~ keep_data_dfbetas(.x, "dm_g", "afdm_g")))
```
...
taxa_unnested_keep_data_merged <- taxa_unnested_outlier_candidates_identified |>
 ...
 # Consolidating all outlier candidates into one vector
   mutate(keep_data = ifelse(keep_data_dm_percentiles == TRUE & keep_data_afdm_percentiles == TRUE & keep_data_cook == TRUE & keep_data_leverage == TRUE & keep_data_stud_resid == TRUE & keep_data_dffits == TRUE & keep_data_dfbetas_int == TRUE & keep_data_dfbetas_slope == TRUE, TRUE, FALSE)) 
```
...
## Saving the data
```{r}
biomass_ethanol_dm_afdm_cleaned <- taxa_unnested_keep_data_merged |>
  select(taxa_clean, dm_g, afdm_g, keep_data)

write_csv(biomass_ethanol_dm_afdm_cleaned, here("data/processed/ethanol/dm_afdm/biomass_ethanol_dm_afdm_cleaned.csv"))
```
