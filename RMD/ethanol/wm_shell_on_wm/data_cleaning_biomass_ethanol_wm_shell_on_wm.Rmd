...
```{r}
step_1 <- min_cutoff(data = biomass_data_taxaclean, wm_shell_on_g, wm_g)  
```

Removing observations when wet mass is less than dry mass (considering these errors in measurement or recording)

```{r}
step_2 <- check_mass_diff(step_1, wm_shell_on_g, wm_g)
```

Filtering ethanol samples only

```{r}
step_3 <- step_2 |>
  filter(preservation_method == "Ethanol")
```
...
cleveland_plots <- step_4 |>
  ...
  mutate(wm_shell_on_plot = map2(.x = data,
                    .y = taxa_clean,
                    ~cleveland_plot(.x, "wm_shell_on_g") + ggtitle(.y)),
         wm_plot = map2(.x = data,
                    .y = taxa_clean,
                    ~cleveland_plot(.x, "wm_g") + ggtitle(.y)))
cleveland_plots$wm_shell_on_plot
...
```{r}
biomass_ethanol_percentiles <- step_4 |>
  group_by(taxa_clean) |>
  mutate(keep_data_wm_shell_on_percentiles = percentiles(wm_shell_on_g, 0.025, 0.975),
         keep_data_wm_percentiles = percentiles(wm_g, 0.025, 0.975))
  
# Checking which observations are identified by the percentile method
biomass_ethanol_percentiles |> 
  filter(keep_data_wm_shell_on_percentiles == FALSE) |> 
  select(taxa_clean, wm_shell_on_g, keep_data_wm_shell_on_percentiles, obs_num) |>
  arrange(taxa_clean, wm_shell_on_g) |>
  print(n = Inf)
```
...
```{r}
#Nesting by taxa
taxa_nested <- biomass_ethanol_percentiles |> 
  select(taxa_clean, wm_shell_on_g, wm_g, obs_num, keep_data_wm_shell_on_percentiles, keep_data_wm_percentiles) |> 
  nest(data = c(wm_shell_on_g, wm_g, obs_num, keep_data_wm_shell_on_percentiles, keep_data_wm_percentiles))
...
taxa_nested_outlier_candidates_identified <- taxa_nested |> 
  mutate(keep_data_leverage = map(.x = data,
                                  ~ keep_data_leverage(.x, "wm_shell_on_g", "wm_g")),
         keep_data_stud_resid = map(.x = data,
                                    ~ keep_data_stud_resid(.x, "wm_shell_on_g", "wm_g")),
         keep_data_cook = map(.x = data,
                              ~ keep_data_cook(.x, "wm_shell_on_g", "wm_g")),
         keep_data_dffits = map(.x = data,
                                ~ keep_data_dffits(.x, "wm_shell_on_g", "wm_g")),
         keep_data_dfbetas = map(.x = data,
                                ~ keep_data_dfbetas(.x, "wm_shell_on_g", "wm_g")))
```
...
taxa_unnested_keep_data_merged <- taxa_unnested_outlier_candidates_identified |>
 ...
 # Consolidating all outlier candidates into one vector
   mutate(keep_data = ifelse(keep_data_wm_shell_on_percentiles == TRUE & keep_data_wm_percentiles == TRUE & keep_data_cook == TRUE & keep_data_leverage == TRUE & keep_data_stud_resid == TRUE & keep_data_dffits == TRUE & keep_data_dfbetas_int == TRUE & keep_data_dfbetas_slope == TRUE, TRUE, FALSE)) 
```
...
## Saving the data
```{r}
biomass_ethanol_wm_shell_on_wm_cleaned <- taxa_unnested_keep_data_merged |>
  select(taxa_clean, wm_shell_on_g, wm_g, keep_data)

write_csv(biomass_ethanol_wm_shell_on_wm_cleaned, here("data/processed/ethanol/wm_shell_on_wm/biomass_ethanol_wm_shell_on_wm_cleaned.csv"))
```
