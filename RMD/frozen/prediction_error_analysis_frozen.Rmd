...
```{r cars}
pred_mass_frozen_wm_dm <- taxa_nested_lm_ln_coef_sf_frozen_wm_dm |>
  ...
  mutate(total_dm = map(.x = data,
         ...
         reg_pred_dm_percent_error = map2(.x = total_dm,
                                          .y = reg_pred_dm,
                                          .f = ~-((.x-.y)/.x)*100),
         sma_pred_dm = pmap(.l = list(data, a_sma, slope_sma, sf_sma),
                            .f = ~ sum(..2*(..1$wm_g)^..3)*..4),
         sma_pred_dm_percent_error = map2(.x = total_dm,
                                          .y = sma_pred_dm,
                                          .f = ~-((.x-.y)/.x)*100),
...
pred_mass_frozen_wm_dm_error <- pred_mass_frozen_wm_dm |>
  select(taxa_clean, total_dm, reg_pred_dm, reg_pred_dm_percent_error, 
         sma_pred_dm, sma_pred_dm_percent_error,
         ratio_pred_dm, ratio_pred_dm_percent_error) |>
  unnest(cols = c(total_dm, reg_pred_dm, reg_pred_dm_percent_error, 
                  sma_pred_dm, sma_pred_dm_percent_error,
                  ratio_pred_dm, ratio_pred_dm_percent_error))

pred_mass_frozen_wm_dm_error_long <- pred_mass_frozen_wm_dm_error |>
  select(taxa_clean, reg_pred_dm_percent_error, sma_pred_dm_percent_error,
         ratio_pred_dm_percent_error) |>
  pivot_longer(cols = c(reg_pred_dm_percent_error, sma_pred_dm_percent_error,
                        ratio_pred_dm_percent_error), 
               names_to = "pred_method", values_to = "percent_error")
  
ggplot(data = pred_mass_frozen_wm_dm_error_long, aes(x = pred_method, y = percent_error)) +
