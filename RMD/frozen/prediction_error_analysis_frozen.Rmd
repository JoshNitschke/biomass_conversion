...
total_pred_mass_frozen_wm_dm <- taxa_nested_lm_ln_coef_sf_frozen_wm_dm |>
  ...
  mutate(total_dm = map(.x = data,
         ...
         total_ratio_pred_dm_percent_error = map2(.x = total_dm,
                                          ...
                                          .f = ~-((.x-.y)/.x)*100))
         # total_median_ratio_pred_dm = map2(.x = data,
         #                      .y = biomass_frozen_wm_dm_cleaned_ratios$median_dm_wm,
         #                      .f = ~ sum(.y*.x$wm_g)),
         # total_median_ratio_pred_dm_percent_error = map2(.x = total_dm,
         #                                  .y = total_median_ratio_pred_dm,
         #                                  .f = ~-((.x-.y)/.x)*100))
...
total_pred_mass_frozen_wm_dm_error <- total_pred_mass_frozen_wm_dm |>
  select(taxa_clean, total_dm, total_reg_pred_dm, total_reg_pred_dm_percent_error, 
         ...
         #total_median_ratio_pred_dm, total_median_ratio_pred_dm_percent_error) |>
...
total_pred_mass_frozen_wm_dm_error_long <- total_pred_mass_frozen_wm_dm_error |>
  select(taxa_clean, total_reg_pred_dm_percent_error, 
         #total_sma_pred_dm_percent_error,
         total_ratio_pred_dm_percent_error) |> 
         #total_median_ratio_pred_dm_percent_error) |>
  pivot_longer(cols = c(total_reg_pred_dm_percent_error, 
                        #total_sma_pred_dm_percent_error,
                        total_ratio_pred_dm_percent_error),
                        #total_median_ratio_pred_dm_percent_error), 
               names_to = "pred_method", values_to = "percent_error")
  
total_percentage_error_plot <- ggplot(data = total_pred_mass_frozen_wm_dm_error_long, aes(x = pred_method, y = percent_error)) +
  ...
  scale_x_discrete(labels = c(#"median ratio",
                              "mean ratio", 
...
```{r}
png(here("output/png/total_percentage_error_plot.png"), width = 2000, height = 1400, res = 300)
par(omi = c(1,1,1,1))  
total_percentage_error_plot
dev.off()
