...
```{r}
biomass_frozen_count_wm_ratios <- biomass_frozen_count_wm_cleaned |>
...
write_csv(biomass_frozen_count_wm_ratios,
...
```{r}
pred_mass_frozen_count_wm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_wm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_wm = map2(.x = data,
                              .y = biomass_frozen_count_wm_ratios$mean_wm_count,
...
```{r}
pred_mass_frozen_count_wm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_wm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$individual_count)^..3*..4),
         ols_pred_wm_percent_error = map2(.x = data,
                                          .y = ols_pred_wm,
...
```{r}
summary_frozen_count_wm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_frozen_count_wm_ratios) |>
  left_join(pred_mass_frozen_count_wm_error_eval)
...
summary_frozen_count_wm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
  ...
  left_join(pred_mass_frozen_count_wm_outliers_removed_error_eval) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_frozen_count_wm <- bind_rows(summary_frozen_count_wm_ratios, summary_frozen_count_wm_ratios_outliers_removed) |>
  arrange(taxa_clean)
