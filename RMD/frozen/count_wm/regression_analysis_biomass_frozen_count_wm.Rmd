...
```{r}
taxa_nested_sma <- biomass_frozen_count_wm_cleaned |> 
...
```{r}
taxa_nested_sma_outliers_removed <- biomass_frozen_count_wm_cleaned |> 
...
```{r}
taxa_nested_power_analysis_check <- biomass_frozen_count_wm_cleaned |> 
  group_nest(taxa_clean) |>
...
```{r}
taxa_nested_sma_log10_plot <- biomass_frozen_count_wm_cleaned |> 
  group_nest(taxa_clean) |>
...
```{r}
taxa_nested_outliers_removed_sma_log10_plot <- biomass_frozen_count_wm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(sma_log10_object = map(.x = data,
                          ~ sma(wm_g ~ individual_count, log = "xy", data = .x)),
