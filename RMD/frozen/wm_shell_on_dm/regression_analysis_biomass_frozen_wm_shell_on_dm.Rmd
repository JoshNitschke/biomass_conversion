...
```{r}
biomass_frozen_wm_shell_on_dm_cleaned <- read_csv(here("data/processed/frozen/wm_shell_on_dm/biomass_frozen_wm_shell_on_dm_cleaned.csv"))
```

## Running lm's on untransformed data

```{r}
taxa_nested_lm <- biomass_frozen_wm_shell_on_dm_cleaned |> 
...
```{r}
taxa_nested_lm_outliers_removed <- biomass_frozen_wm_shell_on_dm_cleaned |> 
...
```{r}
taxa_nested_lm_ln_plot <- biomass_frozen_wm_shell_on_dm_cleaned |> 
  group_nest(taxa_clean) |>
...
write_csv(reg_summary_a_sf,
          here("output/frozen/wm_shell_on_dm/biomass_regression_summary_frozen_wm_shell_on_dm.csv"))
```
...
```{r}
biomass_frozen_wm_shell_on_dm_ratios <- biomass_frozen_wm_shell_on_dm_cleaned |>
...
write_csv(biomass_frozen_wm_shell_on_dm_ratios,
          here("output/frozen/wm_shell_on_dm/biomass_ratio_summary_frozen_wm_shell_on_dm.csv"))
```
...
```{r}
pred_mass_frozen_wm_shell_on_dm_error_eval <- taxa_nested_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
         ...
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_frozen_wm_shell_on_dm_ratios$mean_dm_wm_shell_on,
                              .f = ~ .y*.x$wm_shell_on_g),
...
```{r}
taxa_nested_outliers_removed_lm_ln_plot <- biomass_frozen_wm_shell_on_dm_cleaned |> 
  filter(keep_data == TRUE) |>
  group_nest(taxa_clean) |>
  mutate(lm_ln_object = map(.x = data,
...
write_csv(reg_summary_outliers_removed_a_sf,
          here("output/frozen/wm_shell_on_dm/biomass_regression_summary_frozen_wm_shell_on_dm_outliers_removed.csv"))
```
...
```{r}
biomass_frozen_wm_shell_on_dm_outliers_removed_ratios <- biomass_frozen_wm_shell_on_dm_cleaned |>
  filter(keep_data == TRUE) |>
  mutate(dm_wm_shell_on = dm_g/wm_shell_on_g) |>
  group_nest(taxa_clean) |>
  mutate(dm_wm_shell_on_t_test = map(.x = data,
...
# Saving the ratio summary
write_csv(biomass_frozen_wm_shell_on_dm_outliers_removed_ratios,
          here("output/frozen/wm_shell_on_dm/biomass_ratio_summary_frozen_wm_shell_on_dm_outliers_removed.csv"))
```
...
```{r}
pred_mass_frozen_wm_shell_on_dm_outliers_removed_error_eval <- taxa_nested_outliers_removed_lm_ln_plot_coef_CI_BT |>
  mutate(ols_pred_dm = pmap(.l = list(data, a, slope, sf),
                            .f = ~ ..2*(..1$wm_shell_on_g)^..3*..4),
         ols_pred_dm_percent_error = map2(.x = data,
                                          .y = ols_pred_dm,
                                          .f = ~-((.x$dm_g-.y)/.x$dm_g)*100),
         ratio_pred_dm = map2(.x = data,
                              .y = biomass_frozen_wm_shell_on_dm_outliers_removed_ratios$mean_dm_wm_shell_on,
                              .f = ~ .y*.x$wm_shell_on_g),
         ratio_pred_dm_percent_error = map2(.x = data,
                                          .y = ratio_pred_dm,
                                          .f = ~-((.x$dm_g-.y)/.x$dm_g)*100),
...
```{r}
summary_frozen_wm_shell_on_dm_ratios <- left_join(reg_summary_a_sf, 
                                         biomass_frozen_wm_shell_on_dm_ratios) |>
  left_join(pred_mass_frozen_wm_shell_on_dm_error_eval)

summary_frozen_wm_shell_on_dm_ratios_outliers_removed <- left_join(reg_summary_outliers_removed_a_sf,
                                                          biomass_frozen_wm_shell_on_dm_outliers_removed_ratios) |>
  left_join(pred_mass_frozen_wm_shell_on_dm_outliers_removed_error_eval) |>
  mutate(taxa_clean = paste0(taxa_clean, "_outliers_removed"))

full_summary_frozen_wm_shell_on_dm <- bind_rows(summary_frozen_wm_shell_on_dm_ratios, summary_frozen_wm_shell_on_dm_ratios_outliers_removed) |>
  arrange(taxa_clean)

write_csv(full_summary_frozen_wm_shell_on_dm,
          here("output/frozen/wm_shell_on_dm/full_summary_frozen_wm_shell_on_dm.csv"))
```