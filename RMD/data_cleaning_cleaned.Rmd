...
step_4 |> 
  filter(taxa_clean == taxa) |> 
  ggplot(aes(x = wm_g, y = obs_num, label = obs_num)) +
...
taxa_unnested_outlier_candidates_identified <- taxa_nested_outlier_candidates_identified |>
  unnest(cols = c(data, leverage, keep_data_stud_resid,
  keep_data_cook, keep_data_dffits, keep_data_dfbetas)) 

taxa_unnested_outlier_candidates_identified |>
  filter(taxa_clean == "Amphipoda") |>
  select(leverage_values, keep_data_leverage, obs_num) |>
  arrange(desc(leverage_values))|> 
  print(n = Inf) |>
