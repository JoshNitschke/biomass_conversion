...
```{r}
mean_dm <- mean(biomass_wobs$dm_g, na.rm = TRUE)
sd_dm <- sd(biomass_wobs$dm_g, na.rm = TRUE)
...
### Generally good practice to ungroup() after group_by()
# biomass_wobs |>  
#   group_by(taxa_clean) |> 
#             mutate(z_dm = scale(dm_g, center = TRUE, scale = TRUE),
#             z_wm = scale(wm_g, center = TRUE, scale = TRUE)) |> 
...
taxa_lgl |>
  ...
  unnest(cols = leverage, stud_residual, cooks_d, dffits, dfbetas, keep_data_cook, keep_data_leverage, keep_data_stud_resid, keep_data_dffits, keep_data_dfbetas, taxa_clean) |>
  filter(taxa_clean == "Amphipoda") |>
...
#trying some fancy shit
#failed :'(
x <- list(taxa_cook$leverage)
y <- list(taxa_cook$n)
z <- list(taxa_cook$predictors)
taxa_lgl |> mutate(keep_data_leverage = pmap(list(x, y, z),
                                             ~x < 2*(z+1)/y))
...
# Filter out values less than what was determined by cooks d
taxa_unnested <- taxa_unnested |>
  ...
  mutate(keep_data = ifelse(keep_data_wm_percentiles == TRUE & keep_data_dm_percentiles == TRUE & keep_data_cook == TRUE & keep_data_leverage == TRUE & keep_data_stud_resid == TRUE & keep_data_dffits == TRUE & keep_data_dfbetas == TRUE, TRUE, FALSE)) 

taxa_unnested |> 
  pull(keep_data) |> 
  janitor::tabyl()

taxa_unnested$keep_data
