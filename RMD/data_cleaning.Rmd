...
taxa_lgl <- taxa_cook |> 
  mutate(keep_data = map2(.x = cooks_d,
                          .y = df_residual,
...
taxa_lgl |> 
  select(data, keep_data) |> 
  unnest(cols = c(data, keep_data)) |> 
  nrow()
...
```{r}
write_csv(thing_you_want_tosave, here("data/processed/biomass_conversions_frozen_nooutliers.csv"))
```
