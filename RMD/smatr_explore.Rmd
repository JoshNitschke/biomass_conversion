...
```{r}
plot(fitted(log10_dry_wet_sma), residuals(log10_dry_wet_sma))
plot(log10_dry_wet_sma, which = "residual") #alternative
```
#### Normality
```{r}
...
biomass_frozen |> 
  ...
  mutate(log_dm_g = log10(dm_g),
         log_wm_g = log10(wm_g)) |>
           ggplot(aes(x = wm_g, y= dm_g)) +
  geom_point(alpha = 0.50) + facet_wrap(~ taxa, ncol = 4, scales = "free") + stat_smooth(method = "lm")
```

```{r, fig.width = 12, fig.height = 8}
#untransformed only taxa with n >= 10
biomass_frozen |> 
  group_by(taxa) |>
  filter(wm_g >= 0.001, dm_g >=0.001) |>
  select(dm_g, wm_g) |> 
...
biomass_frozen |> 
  ...
  mutate(log_dm_g = log10(dm_g),
         log_wm_g = log10(wm_g)) |>
    filter(n() >= 10)|>
  ggplot(aes(x = log_wm_g, y= log_dm_g)) +
  geom_point(alpha = 0.50) + facet_wrap(~ taxa, ncol = 4, scales = "free") + stat_smooth(method = "lm")
